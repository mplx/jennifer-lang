...
#
# ratelimit.j - a fixed-window rate limiter on memcached, the sharpest use of
# memcached's distinctive strength: atomic `incr` plus a per-key TTL. Each key
# (an IP, a user, an API token) counts hits in a time window; when the counter
# is first created it is armed with the window's expiry, so it resets on its own
# when the window ends - nothing to reap. Built on the `memcache` module, so it
...
#     }
#
# Fixed window: the count resets when the window expires, so a burst can span a
# window boundary. A sliding window / token bucket is a later refinement.
use convert;
import "./memcache.j" as memcache;
...
# allow records one hit against `key` and reports whether it is within `limit`
# for the current `window` (seconds). The window starts at the first hit: an
# absent counter is created with the window's TTL, and the counter expires on
# its own when the window ends.
...
# remaining reports how many hits are left for `key` in the current window
# (the full `limit` when the window has no hits yet, 0 once it is exhausted).
