...
| `internal/interpreter` | Full programs in-memory with stdout captured; value-semantics aliasing (eager deep copies) under `-race`; `CallByName` / `CallByNameWith` dispatch and `RaiseError` classification                                      |
