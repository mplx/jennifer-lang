...
| `net`     | `use net;`     | M16.2: TCP + UDP sockets and DNS. TCP `connect`/`listen`/`accept`/`readBytes`/`writeBytes`/`eof`, UDP `listenUDP`/`sendTo`/`recvFrom`, DNS `lookup`/`reverseLookup`, polymorphic `close`/`address`; structs `net.Conn`, `net.Listener`, `net.UDPSocket`, `net.Datagram`. TinyGo binary returns friendly errors; use `jennifer-go` for real net I/O | [libraries/net.md](../libraries/net.md) |
